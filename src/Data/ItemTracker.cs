    ...
    public class ItemTracker {
        ...
        private static List<string> GetMysterySeedSettingsForSpoilerLog() {
            ...
            List<string> MysterySettings = new List<string>() {
                ...
                $"\t- Hexagon Quest: {GetBool(HexagonQuestEnabled)}",
                GetBool(HexagonQuestEnabled) ? $"\t\t- Hexagon Quest Goal: {SaveFile.GetInt(HexagonQuestGoal)}" : "",
                GetBool(HexagonQuestEnabled) ? $"\t\t- Extra Hexagons: {SaveFile.GetInt(HexagonQuestExtras)}%" : "",
                GetBool(HexagonQuestEnabled) ? $"\t\t- Hexagon Quest Ability Shuffle Type: {(GetBool(HexagonQuestPageAbilities) ? "PAGES" : "HEXAGONS")}" : "",
                $"\t- Sword Progression: {GetBool(SwordProgressionEnabled)}",
                $"\t- Keys Behind Bosses: {GetBool(KeysBehindBosses)}",
                $"\t- Start with Sword: {GetBool(StartWithSword)}",
                $"\t- Shuffled Abilities: {GetBool(AbilityShuffle)}",
                $"\t- Shuffled Ladders: {GetBool(LadderRandoEnabled)}",
                $"\t- Shuffled Breakables: {GetBool(BreakableShuffleEnabled)}",
                $"\t- Grass Randomizer: {GetBool(GrassRandoEnabled)}",
                $"\t- Shuffle Fuses: {GetBool(FuseShuffleEnabled)}",
                $"\t- Shuffle Bells: {GetBool(BellShuffleEnabled)}",
                $"\t- Shuffle Enemy Drops: {GetBool(ShuffleEnemyDropsEnabled)}",
                $"\t\t- Extra Enemy Drops: {GetBool(ExtraEnemyDropsEnabled)}",
                $"\t- Shuffle Enemy Souls: {GetBool(ShuffleEnemySoulsEnabled)}",
                $"\t- Entrance Randomizer: {GetBool(EntranceRando)}",
                GetBool(EntranceRando) ? $"\t\t- Fewer Shops: {GetBool(ERFixedShop)}" : "",
                GetBool(EntranceRando) ? $"\t\t- Matching Direction: {GetBool(PortalDirectionPairs)}" : "",
                GetBool(EntranceRando) ? $"\t\t- Decoupled: {GetBool(Decoupled)}" : "",
                GetBool(EntranceRando) ? $"\t\t- Fox Prince: {GetBool(FoxPrinceEnabled)}" : "",
                $"\t- Laurels Location: {((RandomizerSettings.FixedLaurelsType)SaveFile.GetInt(LaurelsLocation)).ToString()}",
                $"\t- Maskless Logic: {GetBool(MasklessLogic)}",
                $"\t- Lanternless Logic: {GetBool(LanternlessLogic)}",
                $"\t- Ladder Storage Logic: {((RandomizerSettings.LadderStorageType)SaveFile.GetInt(LadderStorageDifficulty)).ToString()}",
                $"\t\t- Ladder Storage Without Items: {GetBool(LadderStorageWithoutItems)}",
                $"\t- Ice Grapple Logic: {((RandomizerSettings.IceGrapplingType)SaveFile.GetInt(IceGrapplingDifficulty)).ToString()}",
                $"\t- Laurels Zips: {GetBool(LaurelsZips)}\n",
