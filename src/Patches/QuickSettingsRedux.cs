...
namespace TunicRandomizer {
    public class QuickSettingsRedux : MonoBehaviour {
        ...
        private static int AdvancedOptionsPage = 0;
        ...
        private static bool showMusic = false;
        ...
        private static Rect scRect(float x, float y, float width, float height, bool scaleX = true, bool scaleY = true, bool scaleW = true, bool scaleH = true) {
        ...
        private void OnGUI() {
            if (SceneManager.GetActiveScene().name == "TitleScreen" && GameObject.FindObjectOfType<TitleScreen>() != null) {
                ...
                windowRects["singlePlayer"] = new Rect(20f, (float)Screen.height * 0.12f, 658 * guiScale, y);
                GUI.Window(101, windowRects["singlePlayer"], new Action<int>(SinglePlayerQuickSettingsWindow), "Quick Settings Redux");
                ...
                GameObject.Find("elderfox_sword graphic").transform.localPosition = new Vector3(-14.3f, 23.1f, -1.1f);
            }
        }
        ...
        private static void ResetButtons(bool logic = false, bool general = false, bool hints = false, bool enemyRando = false, bool music = false, bool misc = false) {
            showLogic = logic;
            showGeneral = general;
            showHints = hints;
        ...
        private static void ArchipelagoQuickSettingsWindow(int windowID) {
            ...
            apHeight += 40f * guiScale;
            GUI.Label(new Rect(10f * guiScale, apHeight, 500f * guiScale, 30f * guiScale), $"Player: {(TunicRandomizer.Settings.ConnectionSettings.Player)}");
            apHeight += 40f * guiScale;
        ...
        private static void SinglePlayerQuickSettingsWindow(int windowID) {
            ...
            switch (TunicRandomizer.Settings.Mode) {
                case RandomizerSettings.RandomizerType.SINGLEPLAYER:
                    bool ShowLogic = GUI.Button(scRect(10f, y, 98f, 30f), "Seed");
                    if (ShowLogic) {
                        ResetButtons(logic: true);
                    }
                    break;
                case RandomizerSettings.RandomizerType.ARCHIPELAGO:
                    GUI.skin.button.fontSize = (int)(18.5 * guiScale);
                    bool ShowArchipelago = GUI.Button(scRect(10f, y, 98f, 30f), "Archipelago");
                    GUI.skin.button.fontSize = (int)(20 * guiScale);
                    if (ShowArchipelago) {
                        ResetButtons(logic: true);
                    }
            ...
            bool ShowGeneral = GUI.Button(scRect(118f, y, 98f, 30f), "General");
            if (ShowGeneral) {
                ResetButtons(general: true);
            }
            GUI.backgroundColor = showHints ? Color.black : Color.white;
            bool ShowHints = GUI.Button(scRect(226f, y, 98f, 30f), "Hints");
            if (ShowHints) {
                ResetButtons(hints: true);
            }
            GUI.backgroundColor = showEnemyRando ? Color.black : Color.white;
            GUI.skin.button.fontSize = (int)(15f * guiScale);
            bool ShowEnemyRando = GUI.Button(scRect(334f, y, 98f, 30f), "Enemy Rando");
            if (ShowEnemyRando) {
                ResetButtons(enemyRando: true);
            }
            GUI.skin.button.fontSize = (int)(20f * guiScale);
            GUI.backgroundColor = showMusic ? Color.black : Color.white;
            bool ShowMusic = GUI.Button(scRect(442f, y, 98f, 30f), "Music");
            if (ShowMusic) {
                ResetButtons(music: true);
            }
            GUI.backgroundColor = showMisc ? Color.black : Color.white;
            bool ShowMisc = GUI.Button(scRect(550f, y, 98f, 30f), "Fox");
            if (ShowMisc) {
            ...
            if (showEnemyRando) { label = "Enemy Randomizer"; }
            if (showMusic) { label = "Music Shuffle"; }
            if (showMisc) { label = "Miscellaneous"; }
        ...
        private static float ArchipelagoSettingsSection(float y) {
            //GUI.Label(new Rect(10f * guiScale, y, 400f * guiScale, 30f * guiScale), $"Archipelago Settings");
            //y += 40f * guiScale;
            if (ShowAPSettingsWindow) {
            ...
            if (Archipelago.instance.integration != null && Archipelago.instance.integration.connected) {
                ...
                GUI.Label(new Rect(250f * guiScale, y, 400f * guiScale, 30f * guiScale), $"(world {Archipelago.instance.integration.session.ConnectionInfo.Slot} of {playerCount})");
            } else {
                GUI.color = Color.red;
            ...
            bool ConnectViaRoomLink = GUI.Button(ShowTooltip(new Rect(430f * guiScale, y, 235f * guiScale, 30f * guiScale), "singlePlayer", "Connect via Room Link"), "Connect via Room Link");
        ...
        private static float ArchipelagoConnectionSettings(float y) {
            GUI.skin.label.fontSize = (int)(25 * guiScale);
            GUI.skin.button.fontSize = (int)(17 * guiScale);

            //Player name
            GUI.Label(new Rect(10f * guiScale, y, 300f * guiScale, 30f * guiScale), $"Host: {textWithCursor(getConnectionSetting("Hostname"), editingFlags["Hostname"], true)}");
            
            GUI.Label(new Rect(334f * guiScale, y, 300f * guiScale, 30f * guiScale), $"Port: {textWithCursor(getConnectionSetting("Port"), editingFlags["Port"], showPort)}");
            showPort = GUI.Toggle(new Rect(575f * guiScale, y + (10f * guiScale), 75f * guiScale, 30f * guiScale), showPort, "Show");

            y += 40f * guiScale;
            //bool setLocalhost = GUI.Toggle(new Rect(160f * guiScale, y, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.ConnectionSettings.Hostname == "localhost", "localhost");
            //if (setLocalhost && TunicRandomizer.Settings.ConnectionSettings.Hostname != "localhost") {
            //    setConnectionSetting("Hostname", "localhost");
            //    RandomizerSettings.SaveSettings();
            //}

            //bool setArchipelagoHost = GUI.Toggle(new Rect(10f * guiScale, y, 140f * guiScale, 30f * guiScale), TunicRandomizer.Settings.ConnectionSettings.Hostname == "archipelago.gg", "archipelago.gg");
            ...
            //y += 40f * guiScale;
            bool EditHostname = GUI.Button(new Rect(10f * guiScale, y, 75f * guiScale, 30f * guiScale), editingFlags["Hostname"] ? "Save" : "Edit");
            if (EditHostname) handleEditButton("Hostname");

            bool PasteHostname = GUI.Button(new Rect(100f * guiScale, y, 75f * guiScale, 30f * guiScale), "Paste");
            if (PasteHostname) handlePasteButton("Hostname");

            bool ClearHostname = GUI.Button(new Rect(190f * guiScale, y, 75f * guiScale, 30f * guiScale), "Clear");
            if (ClearHostname) handleClearButton("Hostname");
            //Port


            bool EditPort = GUI.Button(new Rect(334f * guiScale, y, 75f * guiScale, 30f * guiScale), editingFlags["Port"] ? "Save" : "Edit");
            if (EditPort) handleEditButton("Port");

            bool PastePort = GUI.Button(new Rect(424f * guiScale, y, 75f * guiScale, 30f * guiScale), "Paste");
            if (PastePort) handlePasteButton("Port");

            bool ClearPort = GUI.Button(new Rect(514f * guiScale, y, 75f * guiScale, 30f * guiScale), "Clear");
            if (ClearPort) handleClearButton("Port");

            //Hostname
            y += 40f * guiScale;
            GUI.Label(new Rect(10f * guiScale, y, 300f * guiScale, 30f * guiScale), $"Player: {textWithCursor(getConnectionSetting("Player"), editingFlags["Player"], true)}");
            GUI.Label(new Rect(334f * guiScale, y, 300f * guiScale, 30f * guiScale), $"Password: {textWithCursor(getConnectionSetting("Password"), editingFlags["Password"], showPassword)}");
            showPassword = GUI.Toggle(new Rect(575f * guiScale, y + (10f * guiScale), 75f * guiScale, 30f * guiScale), showPassword, "Show");

            y += 40f * guiScale;
            bool EditPlayer = GUI.Button(new Rect(10f * guiScale, y, 75f * guiScale, 30f * guiScale), editingFlags["Player"] ? "Save" : "Edit");
            if (EditPlayer) handleEditButton("Player");

            bool PastePlayer = GUI.Button(new Rect(100f * guiScale, y, 75f * guiScale, 30f * guiScale), "Paste");
            if (PastePlayer) handlePasteButton("Player");

            bool ClearPlayer = GUI.Button(new Rect(190f * guiScale, y, 75f * guiScale, 30f * guiScale), "Clear");
            if (ClearPlayer) handleClearButton("Player");

            //Password

            bool EditPassword = GUI.Button(new Rect(334f * guiScale, y, 75f * guiScale, 30f * guiScale), editingFlags["Password"] ? "Save" : "Edit");
            if (EditPassword) handleEditButton("Password");

            bool PastePassword = GUI.Button(new Rect(424f * guiScale, y, 75f * guiScale, 30f * guiScale), "Paste");
            if (PastePassword) handlePasteButton("Password");

            bool ClearPassword = GUI.Button(new Rect(514f * guiScale, y, 75f * guiScale, 30f * guiScale), "Clear");
            if (ClearPassword) handleClearButton("Password");
            GUI.skin.button.fontSize = (int)(20 * guiScale);
            return y;
        }

        private static float ArchipelagoWorldSettings(float y) {
            return y;
        }

        private static float LogicSettingsSection(float y) {
            GUI.Label(new Rect(10f * guiScale, y, 400f * guiScale, 30f * guiScale), $"Logic Settings");
            GUI.backgroundColor = Color.white;
            y += 40f * guiScale;
            bool ToggleHexagonQuest = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Hexagon Quest"), TunicRandomizer.Settings.GameMode == RandomizerSettings.GameModes.HEXAGONQUEST, $"Hexagon Quest {(TunicRandomizer.Settings.GameMode == RandomizerSettings.GameModes.HEXAGONQUEST ? $"(<color=#E3D457>{(TunicRandomizer.Settings.RandomizeHexQuest ? "?" : TunicRandomizer.Settings.HexagonQuestGoal.ToString())}</color>)" : "")}");
            ...
            TunicRandomizer.Settings.SwordProgressionEnabled = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Sword Progression"), TunicRandomizer.Settings.SwordProgressionEnabled, "Sword Progression");
            TunicRandomizer.Settings.KeysBehindBosses = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Keys Behind Bosses"), TunicRandomizer.Settings.KeysBehindBosses, "Keys Behind Bosses");
            y += 40f * guiScale;
            TunicRandomizer.Settings.ShuffleAbilities = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Shuffle Abilities"), TunicRandomizer.Settings.ShuffleAbilities, "Shuffle Abilities");
            TunicRandomizer.Settings.EntranceRandoEnabled = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Entrance Randomizer"), TunicRandomizer.Settings.EntranceRandoEnabled, "Entrance Randomizer");
            TunicRandomizer.Settings.ShuffleLadders = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Shuffle Ladders"), TunicRandomizer.Settings.ShuffleLadders, "Shuffle Ladders");
            y += 40f * guiScale;
            TunicRandomizer.Settings.GrassRandomizer = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Grass Randomizer"), TunicRandomizer.Settings.GrassRandomizer, "Grass Randomizer");
            TunicRandomizer.Settings.BreakableShuffle = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Shuffle Breakable Objects"), TunicRandomizer.Settings.BreakableShuffle, "Shuffle Breakables");
            TunicRandomizer.Settings.FuseShuffle = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Shuffle Fuses"), TunicRandomizer.Settings.FuseShuffle, "Shuffle Fuses");
            y += 40f * guiScale;
            TunicRandomizer.Settings.BellShuffle = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Shuffle Bells"), TunicRandomizer.Settings.BellShuffle, "Shuffle Bells");
            TunicRandomizer.Settings.StartWithSwordEnabled = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Start With Sword"), TunicRandomizer.Settings.StartWithSwordEnabled, "Start With Sword");
            TunicRandomizer.Settings.MysterySeed = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Mystery Seed"), TunicRandomizer.Settings.MysterySeed, "Mystery Seed");
            y += 40f * guiScale;
            GUI.Label(new Rect(10f * guiScale, y, 300f * guiScale, 30f * guiScale), $"Custom Seed: {(CustomSeed == "" ? "Not Set" : CustomSeed.ToString())}");
            //bool ClearSeed = GUI.Button(new Rect(430f * guiScale, y, 200f * guiScale, 30f * guiScale), "Clear Seed");
            //if (ClearSeed) {
            //    CustomSeed = "";
            //}
            y += 40f * guiScale;
            bool GenerateSeed = GUI.Button(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Generate Seed"), "Generate Seed");
            if (GenerateSeed) {
                CustomSeed = new System.Random().Next().ToString();
            }

            bool CopySettings = GUI.Button(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Copy Seed + Settings"), "Copy Seed + Settings");
            if (CopySettings) {
                GUIUtility.systemCopyBuffer = TunicRandomizer.Settings.GetSettingsString();
            }
            bool PasteSettings = GUI.Button(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Paste Seed + Settings"), "Paste Seed + Settings");
            if (PasteSettings) {
        ...
        private static float GeneralSettingsSection(float y) {
            GUI.Label(new Rect(10f * guiScale, y, 400f * guiScale, 30f * guiScale), $"General Settings");
            GUI.backgroundColor = Color.white;
            y += 40f * guiScale;
            TunicRandomizer.Settings.HeirAssistModeEnabled = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Easier Heir Fight"), TunicRandomizer.Settings.HeirAssistModeEnabled, "Easier Heir Fight");
            TunicRandomizer.Settings.ClearEarlyBushes = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Clear Early Bushes"), TunicRandomizer.Settings.ClearEarlyBushes, "Clear Early Bushes");
            GUI.skin.toggle.fontSize = (int)(19 * guiScale);
            TunicRandomizer.Settings.EnableAllCheckpoints = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Enable All Checkpoints"), TunicRandomizer.Settings.EnableAllCheckpoints, "Enable All Checkpoints");
            GUI.skin.toggle.fontSize = (int)(20 * guiScale);
            y += 40f * guiScale;
            TunicRandomizer.Settings.CheaperShopItemsEnabled = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Cheaper Shop Items"), TunicRandomizer.Settings.CheaperShopItemsEnabled, "Cheaper Shop Items");
            TunicRandomizer.Settings.BonusStatUpgradesEnabled = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Bonus Upgrades"), TunicRandomizer.Settings.BonusStatUpgradesEnabled, "Bonus Upgrades");
            GUI.skin.toggle.fontSize = (int)(17 * guiScale);
            TunicRandomizer.Settings.DisableChestInterruption = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Disable Chest Interruption"), TunicRandomizer.Settings.DisableChestInterruption, "Disable Chest Interruption");
            GUI.skin.toggle.fontSize = (int)(20 * guiScale);
            y += 40f * guiScale;
            TunicRandomizer.Settings.ShowRecentItems = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Show Recent Items"), TunicRandomizer.Settings.ShowRecentItems, "Show Recent Items");
            TunicRandomizer.Settings.SkipItemAnimations = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Skip Item Popups"), TunicRandomizer.Settings.SkipItemAnimations, "Skip Item Popups");

            GUI.skin.toggle.fontSize = (int)(18 * guiScale);
            TunicRandomizer.Settings.FasterUpgrades = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Skip Upgrade Animations"), TunicRandomizer.Settings.FasterUpgrades, "Skip Upgrade Animations");
            GUI.skin.toggle.fontSize = (int)(20 * guiScale);
            y += 40f * guiScale;
            TunicRandomizer.Settings.HolyCrossVisualizer = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Holy Cross DDR"), TunicRandomizer.Settings.HolyCrossVisualizer, "Holy Cross DDR");
            TunicRandomizer.Settings.ArachnophobiaMode = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Arachnophobia Mode"), TunicRandomizer.Settings.ArachnophobiaMode, "Arachnophobia Mode");
            TunicRandomizer.Settings.ShowPlayerPosition = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Show Player Position"), TunicRandomizer.Settings.ShowPlayerPosition, "Show Player Position");
            y += 40f * guiScale;
            TunicRandomizer.Settings.MoreSkulls = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "More Skulls"), TunicRandomizer.Settings.MoreSkulls, "More Skulls");
            TunicRandomizer.Settings.CameraFlip = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "???"), TunicRandomizer.Settings.CameraFlip, "???");

            GUI.skin.button.fontSize = (int)(20 * guiScale);
            return y;
        }

        private static float HintSettingsSection(float y) {
            GUI.Label(new Rect(10f * guiScale, y, 400f * guiScale, 30f * guiScale), $"Hint Settings");
            GUI.backgroundColor = Color.white;
            y += 40f * guiScale;
            TunicRandomizer.Settings.HeroPathHintsEnabled = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Path of the Hero"), TunicRandomizer.Settings.HeroPathHintsEnabled, "Path of the Hero");
            TunicRandomizer.Settings.GhostFoxHintsEnabled = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Ghost Foxes"), TunicRandomizer.Settings.GhostFoxHintsEnabled, "Ghost Foxes");
            TunicRandomizer.Settings.SeekingSpellLogic = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Seeking Spell Logic"), TunicRandomizer.Settings.SeekingSpellLogic, "Seeking Spell Logic");
            y += 40f * guiScale;
            TunicRandomizer.Settings.ShowItemsEnabled = GUI.Toggle(ShowTooltip(new Rect(10f * guiScale, y, 206f * guiScale, 60f * guiScale), "singlePlayer", "Freestanding Items Match Contents"), TunicRandomizer.Settings.ShowItemsEnabled, "Freestanding Items Match Contents");
            GUI.skin.toggle.fontSize = (int)(19 * guiScale);
            TunicRandomizer.Settings.ChestsMatchContentsEnabled = GUI.Toggle(ShowTooltip(new Rect(226f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Chests Match Contents"), TunicRandomizer.Settings.ChestsMatchContentsEnabled, "Chests Match Contents");
            GUI.skin.toggle.fontSize = (int)(20 * guiScale);
            TunicRandomizer.Settings.UseTrunicTranslations = GUI.Toggle(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Write Hints In Trunic"), TunicRandomizer.Settings.UseTrunicTranslations, "Write Hints In Trunic");
            y += 50f * guiScale;
            bool OpenEntranceTracker = GUI.Button(ShowTooltip(new Rect(442f * guiScale, y, 206f * guiScale, 30f * guiScale), "singlePlayer", "Entrance Tracker"), "Entrance Tracker");
        ...
        private static void AdvancedLogicOptionsWindow(int windowID) {
            ...
            if (AdvancedOptionsPage == 0) {
                ...
                advHeight += 40f * guiScale;
            } else if (AdvancedOptionsPage == 1) {
                GUI.Label(new Rect(10f * guiScale, advHeight, 300f * guiScale, 30f * guiScale), $"Laurels Zips in Logic");
                ShowTooltip(new Rect(10f * guiScale, advHeight, 385f * guiScale, 80f * guiScale), "advancedSinglePlayer", "Laurels Zips");
                advHeight += 40f * guiScale;
                TunicRandomizer.Settings.LaurelsZips = GUI.Toggle(new Rect(10f * guiScale, advHeight, 200f * guiScale, 30f * guiScale), TunicRandomizer.Settings.LaurelsZips, "Laurels Zips");
                advHeight += 40f * guiScale;
                GUI.Label(new Rect(10f * guiScale, advHeight, 300f * guiScale, 30f * guiScale), $"Ice Grapples in Logic");
                ShowTooltip(new Rect(10f * guiScale, advHeight, 385f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ice Grapples");
                advHeight += 40f * guiScale;
                bool IceGrapplesOff = GUI.Toggle(new Rect(10f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.IceGrappling == RandomizerSettings.IceGrapplingType.OFF, "Off");
                ShowTooltip(new Rect(10f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ice Grapples Off");
                if (IceGrapplesOff) {
                    TunicRandomizer.Settings.IceGrappling = RandomizerSettings.IceGrapplingType.OFF;
                }
                bool IceGrapplesEasy = GUI.Toggle(new Rect(110f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.IceGrappling == RandomizerSettings.IceGrapplingType.EASY, "Easy");
                ShowTooltip(new Rect(110f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ice Grapples Easy");
                if (IceGrapplesEasy) {
                    TunicRandomizer.Settings.IceGrappling = RandomizerSettings.IceGrapplingType.EASY;
                }
                bool IceGrapplesMedium = GUI.Toggle(new Rect(200f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.IceGrappling == RandomizerSettings.IceGrapplingType.MEDIUM, "Medium");
                ShowTooltip(new Rect(200f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ice Grapples Medium");
                if (IceGrapplesMedium) {
                    TunicRandomizer.Settings.IceGrappling = RandomizerSettings.IceGrapplingType.MEDIUM;
                }
                bool IceGrapplesHard = GUI.Toggle(new Rect(290f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.IceGrappling == RandomizerSettings.IceGrapplingType.HARD, "Hard");
                ShowTooltip(new Rect(290f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ice Grapples Hard");
                if (IceGrapplesHard) {
                    TunicRandomizer.Settings.IceGrappling = RandomizerSettings.IceGrapplingType.HARD;
                }
                advHeight += 40f * guiScale;
                GUI.Label(new Rect(10f * guiScale, advHeight, 300f * guiScale, 30f * guiScale), $"Ladder Storage in Logic");
                ShowTooltip(new Rect(10f * guiScale, advHeight, 385f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ladder Storage");
                advHeight += 40f * guiScale;
                bool LadderStorageOff = GUI.Toggle(new Rect(10f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.LadderStorage == RandomizerSettings.LadderStorageType.OFF, "Off");
                ShowTooltip(new Rect(10f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ladder Storage Off");
                if (LadderStorageOff) {
                    TunicRandomizer.Settings.LadderStorage = RandomizerSettings.LadderStorageType.OFF;
                }
                bool LadderStorageEasy = GUI.Toggle(new Rect(110f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.LadderStorage == RandomizerSettings.LadderStorageType.EASY, "Easy");
                ShowTooltip(new Rect(110f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ladder Storage Easy");
                if (LadderStorageEasy) {
                    TunicRandomizer.Settings.LadderStorage = RandomizerSettings.LadderStorageType.EASY;
                }
                bool LadderStorageMedium = GUI.Toggle(new Rect(200f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.LadderStorage == RandomizerSettings.LadderStorageType.MEDIUM, "Medium");
                ShowTooltip(new Rect(200f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ladder Storage Medium");
                if (LadderStorageMedium) {
                    TunicRandomizer.Settings.LadderStorage = RandomizerSettings.LadderStorageType.MEDIUM;
                }
                bool LadderStorageHard = GUI.Toggle(new Rect(290f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), TunicRandomizer.Settings.LadderStorage == RandomizerSettings.LadderStorageType.HARD, "Hard");
                ShowTooltip(new Rect(290f * guiScale, advHeight, 90f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ladder Storage Hard");
                if (LadderStorageHard) {
                    TunicRandomizer.Settings.LadderStorage = RandomizerSettings.LadderStorageType.HARD;
                }
                advHeight += 40f * guiScale;
                TunicRandomizer.Settings.LadderStorageWithoutItems = GUI.Toggle(new Rect(10f * guiScale, advHeight, 300f * guiScale, 30f * guiScale), TunicRandomizer.Settings.LadderStorageWithoutItems, "Ladder Storage Without Items");
                ShowTooltip(new Rect(10f * guiScale, advHeight, 300f * guiScale, 30f * guiScale), "advancedSinglePlayer", "Ladder Storage Without Items");
                advHeight += 40f * guiScale;
            }
            bool OpenTrickLogicWindow = GUI.Button(new Rect(10f * guiScale, advHeight, 385f * guiScale, 30f * guiScale), AdvancedOptionsPage == 0 ? "Trick & Glitch Logic >" : "< Previous Page");
            if (OpenTrickLogicWindow) {
                AdvancedOptionsPage = AdvancedOptionsPage == 0 ? 1 : 0;
            }
            advHeight += 40f * guiScale;
            bool Close = GUI.Button(new Rect(10f * guiScale, advHeight, 200f * guiScale, 30f * guiScale), "Close");
            if (Close) {
                ShowAdvancedSinglePlayerOptions = false;
                AdvancedOptionsPage = 0;
                RandomizerSettings.SaveSettings();
            }
            advHeight += 40f * guiScale;
        }
